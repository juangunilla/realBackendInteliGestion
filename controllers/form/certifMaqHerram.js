...
const {
  CertifMaqHerram,
  CertifMaqHerramHist,
} = require('../../models/form/certifMaqHerram');
...
const postItem = async (req, res) => {
  try {
    ...
    const nuevo = await crearConHistorial(
      ...
      {
        user: req.user,
        entity: 'certif-maq-herram',
        description: 'Creación de Certif Maq. y Herram',
        payload,
      }
    );
    ...
    console.error('Error al crear Certif Maq. y Herram:', error);
...
const updateItem = async (req, res) => {
  try {
    ...
    const actualizado = await CertifMaqHerram.findByIdAndUpdate(
      _id,
      { $set: update },
      {
        new: true,
...
const getItems = async (req, res) => {
  try {
    const data = await CertifMaqHerram.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener Certif Maq. y Herram:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorial = async (req, res) => {
  try {
    const data = await CertifMaqHerramHist.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener historial Certif Maq. y Herram:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorialByClienteEst = async (req, res) => {
  try {
    ...
    console.error('Error al obtener historial filtrado Certif Maq. y Herram:', error);
    return res.status(500).send({
      status: 'error',

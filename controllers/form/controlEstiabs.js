...
const {
  ControlEstiabs,
  ControlEstiabsHist,
} = require('../../models/form/controlEstiabs');
...
const postItem = async (req, res) => {
  try {
    ...
    const nuevo = await crearConHistorial(
      ...
      {
        user: req.user,
        entity: 'control-estiabs',
        description: 'Creación de Control Estiabs',
        payload,
      }
    );
    ...
    console.error('Error al crear Control Estiabs:', error);
...
const updateItem = async (req, res) => {
  try {
    ...
    const actualizado = await ControlEstiabs.findByIdAndUpdate(
      _id,
      { $set: update },
      {
        new: true,
...
const getItems = async (req, res) => {
  try {
    const data = await ControlEstiabs.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener Control Estiabs:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorial = async (req, res) => {
  try {
    const data = await ControlEstiabsHist.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener historial Control Estiabs:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorialByClienteEst = async (req, res) => {
  try {
    ...
    console.error('Error al obtener historial filtrado Control Estiabs:', error);
    return res.status(500).send({
      status: 'error',

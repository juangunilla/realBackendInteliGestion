...
const postItem = async (req, res) => {
  try {
    ...
    const nuevo = await crearConHistorial(
      ...
      {
        user: req.user,
        entity: 'ddjj-cancerigenos',
        description: 'Creación de DDJJ cancerigenos',
        payload,
      }
    );
    ...
    console.error('Error al crear DDJJ cancerigenos:', error);
...
const updateItem = async (req, res) => {
  try {
    ...
    const actualizado = await DdjjCancerigenos.findByIdAndUpdate(
      _id,
      { $set: update },
      {
        new: true,
...
const getItems = async (req, res) => {
  try {
    const data = await DdjjCancerigenos.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener DDJJ cancerigenos:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorial = async (req, res) => {
  try {
    const data = await DdjjCancerigenosHist.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener historial DDJJ cancerigenos:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorialByClienteEst = async (req, res) => {
  try {
    ...
    console.error('Error al obtener historial filtrado DDJJ cancerigenos:', error);
    return res.status(500).send({
      status: 'error',

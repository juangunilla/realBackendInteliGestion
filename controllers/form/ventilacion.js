...
const {
  Ventilacion,
  VentilacionHist,
} = require('../../models/form/ventilacion');
  ...
  try {
    ...
    const nuevo = await crearConHistorial(
      ...
      payload,
      {
        user: req.user,
        entity: 'ventilacion',
        description: 'Creación de Ventilacion',
        payload,
      }
    );
    ...
    console.error('Error al crear Ventilacion:', error);
...
const updateItem = async (req, res) => {
  try {
    ...
    const actualizado = await Ventilacion.findByIdAndUpdate(
      _id,
      { $set: update },
      {
        new: true,
...
const getItems = async (req, res) => {
  try {
    const data = await Ventilacion.find({});
    return res.status(200).send({
      status: 'success',
...
const getHistorial = async (req, res) => {
  try {
    const data = await VentilacionHist.find({});
    return res.status(200).send({
      status: 'success',
      data: normalizeEntregaDocumentacionDocs(data),
    });
  } catch (error) {
    console.error('Error al obtener historial Ventilacion:', error);
    return res.status(500).send({
      status: 'error',
...
const getHistorialByClienteEst = async (req, res) => {
  try {
    ...
    console.error('Error al obtener historial filtrado Ventilacion:', error);
    return res.status(500).send({
      status: 'error',
